...
strdup_line_content(Line* line){
    ...
    size_t gs_interval = line->gap_start - line->content;
    size_t ge_interval = (line->content + line->capacity - 1) - line->gap_end;
    memcpy(str, line->content, gs_interval);
    memcpy(str + gs_interval + 1, line->gap_end + 1, ge_interval);
    return str;
}
